...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        # T = past_action_window_size + future_action_window_size + 1  # 例
        seq_len = past_action_window_size + future_action_window_size  # ← x の T に合わせる

        # Learnable positional embeddings
        # 1: バッチ次元、seq_len: T+1 に合わせる
        self.positional_embedding = nn.Parameter(
            scale * torch.randn(1, seq_len + 1, hidden_size)
        )
