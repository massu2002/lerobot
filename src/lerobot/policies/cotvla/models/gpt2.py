...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        attn_impl = getattr(
            config,
            "attn_implementation",
            getattr(config, "_attn_implementation", "eager"),  # transformers 側の _attn_implementation にフォールバック
        )
        self.attn = GPT2_ATTENTION_CLASSES[attn_impl](config, layer_idx=layer_idx)
        # self.attn = GPT2_ATTENTION_CLASSES[attn_impl](config, layer_idx=layer_idx)
